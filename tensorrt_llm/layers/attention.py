...
from ..functional import (AttentionMaskType, PositionEmbeddingType,
                          ...
                          gpt_attention, gpt_attention_custom, matmul, repeat_interleave, round,
...
class Attention(Module):
    ...
    def forward(self,
        ...
        # NOTE: Size of hidden_states == [batch_size, seq_length, hidden_size]
        ...
        # NOTE: The hidden_states are projected into Query, Key, and Value matrices using Linear transformation
        # Size of qkv == [batch_size, seq_length, 3 * hidden_size]
        # Size of qkv == [batch_size, 1, 3 * hidden_size] when using KV cache
        ...
        #     default_net().plugin_config.remove_input_padding), f"The variable attention_params is {attention_params} which should be None."
        # assert kv_cache_params is None or kv_cache_params.is_valid(
        #     default_net().plugin_config.gpt_attention_plugin)

        # [x] For a custom plugin
        assert attention_params is None or attention_params.is_valid(
            default_net().plugin_config.gpt_attention_custom_plugin,
            default_net().plugin_config.remove_input_padding), f"The variable attention_params is {attention_params.__class__} which should be None."
        assert kv_cache_params is None or kv_cache_params.is_valid(
            default_net().plugin_config.gpt_attention_custom_plugin)
        ...
        # if default_net().plugin_config.gpt_attention_plugin and False:
        # Module -> Network
        '''
        In /examples/llama/build.py
        ...
        if default_net().plugin_config.gpt_attention_custom_plugin:
            ...
            context, past_key_value = gpt_attention_custom(
                qkv=qkv,
            ...
            # NOTE: The hidden_states are projected into Query, Key, and Value matrices using Linear transformation
            # NOTE: Size of qkv == [batch_size, seq_length, 3 * hidden_size]

            '''
            Before 'transpose_for_scores()'
                query : [batch_size, seq_length, hidden_size]
                key : [batch_size, seq_length, kv_size]
                value : [batch_size, seq_length, kv_size]
            After 'transpose_for_scores()'
                query : [batch_size, num_attention_heads, seq_length, attention_head_size]
                key : [batch_size, num_attention_kv_heads, seq_length, attention_head_size]
                value : [batch_size, num_attention_kv_heads, seq_length, attention_head_size]
            '''
            ...
            # NOTE: Matrix Multiplication for attention scores
