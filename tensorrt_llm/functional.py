...
def gpt_attention_custom(
    ...
    assert (alibi_slopes is not None) == (position_embedding_type.is_alibi())
    attn_plg_creator = trt.get_plugin_registry().get_plugin_creator(
        'GPTAttentionCustom', '1', TRT_LLM_PLUGIN_NAMESPACE)
    assert attn_plg_creator is not None
    assert host_context_lengths is not None or not default_net(
    ).plugin_config.remove_input_padding
    assert isinstance(max_context_length, int)
    ...
    p_dtype = default_net().plugin_config.gpt_attention_custom_plugin
    pf_type = trt.PluginField(
        "type_id", np.array([int(str_dtype_to_trt(p_dtype))], np.int32),
        trt.PluginFieldType.INT32)
    mask_type = trt.PluginField("mask_type", np.array([int(mask_type)],
...
def gpt_attention_custom123(
    ...
    assert host_request_types is not None
    assert (alibi_slopes is not None) == (position_embedding_type.is_alibi())
    # For the fisrt part of the custom kernel
    attn_plg_creator_1 = trt.get_plugin_registry().get_plugin_creator(
        'GPTAttentionCustom_1', '1', TRT_LLM_PLUGIN_NAMESPACE)
    assert attn_plg_creator_1 is not None
    # For the second part of the custom kernel
    attn_plg_creator_2 = trt.get_plugin_registry().get_plugin_creator(
        'GPTAttentionCustom_2', '1', TRT_LLM_PLUGIN_NAMESPACE)
    assert attn_plg_creator_2 is not None
    ...
    attn_plug_1 = attn_plg_creator_1.create_plugin("causal_attn_1", pfc)
    attn_plug_2 = attn_plg_creator_2.create_plugin("causal_attn_2", pfc)
    ...
    plug_inputs = [i.trt_tensor for i in plug_inputs]
    # Split the kernel into two parts
    layer_1 = default_trtnet().add_plugin_v2(plug_inputs, attn_plug_1)
    layer_2 = default_trtnet().add_plugin_v2(plug_inputs, attn_plug_2)



    # The output of layer_2 is same to the existing kernel.
    output = _create_tensor(layer_2.get_output(0), layer_2)
    present_key_value = None
    if use_cache and not paged_kv_cache_flag:
        present_key_value = _create_tensor(layer_2.get_output(1), layer_2)
        assert present_key_value is not None
        expected_outputs = 2
    else:
        expected_outputs = 1

    assert layer_2.num_outputs == expected_outputs, \
        f"Plugin outputs number mismatch with expected, got {layer_2.num_outputs}, expected {expected_outputs}"

    if kv_cache_quant_mode.has_int8_kv_cache() and not paged_kv_cache_flag:
        # past key value
        layer_2.get_input(7).set_dynamic_range(-127, 127)
        # present key value
        layer_2.get_output(1).set_dynamic_range(-127, 127)
