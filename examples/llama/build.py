...
def parse_arguments():
    ...
    parser.add_argument('--use_gpt_attention_custom_plugin',
                        nargs='?',
                        const='float16',
                        type=str,
                        default=False,
    ...
    if not args.remove_input_padding:
        ...
        #     )
        if args.use_gpt_attention_custom_plugin:
            logger.warning(
                f"It is recommended to specify --remove_input_padding when using GPT attention plugin"
            )
...
def build_rank_engine(builder: Builder,
    ...
    # [x] For a custom plugin 
    if args.use_gpt_attention_custom_plugin:
        network.plugin_config.set_gpt_attention_custom_plugin(
            dtype=args.use_gpt_attention_custom_plugin)    
