...
//     // Compute relative attention bias on the fly, with relative attention table [head_num/TP, num_buckets] passed
//     in.
...
//         // The offset.
//         const auto q_offset = tensorrt_llm::common::flat_index_strided3(batch_beam_idx, hi, qk_vec_idx, q_stride,
//         Dh);
...
//             q_bias
//                 = vec_conversion<Qk_vec_k, Qk_vec_m>(*reinterpret_cast<const
//                 Qk_vec_m*>(&params.q_bias[q_bias_offset]));
//         }
...
//             k_bias
//                 = vec_conversion<Qk_vec_k, Qk_vec_m>(*reinterpret_cast<const
//                 Qk_vec_m*>(&params.k_bias[k_bias_offset]));
//         }
//     }
...
//     void** k_cache_base_row_ptr = reinterpret_cast<void**>(kvCacheBuffer.getRowPtr(KVIdxType::K_IDX,
//     batch_beam_idx));
...
//     // cyclic_attention_window_size So we need to rebuild the beam_indices if max_attention_window_size is not equal
//     to
...
//                 Tcache* k_cache_batch = reinterpret_cast<Tcache*>(kvCacheBuffer.getKBlockPtr(seqIdx,
//                 valid_time_now));
...
//         }
//         // The input length;
//         const int input_length_ = MULTI_BLOCK_FLAG ? beam0_context_length % timesteps_per_block :
//         beam0_context_length;
...
//     void** v_cache_base_row_ptr = reinterpret_cast<void**>(kvCacheBuffer.getRowPtr(KVIdxType::V_IDX,
//     batch_beam_idx));
...
//         // Take all previous cache as context when we have no beam searching in order to batch as many LDGs as
//         possible. const int context_length
//             = DO_CROSS_ATTENTION ? kv_loop_length : (HAS_BEAMS ? beam0_context_length : kv_loop_length);
...
//                     // Note that fma will convert 8bit vec to the accumulation data type (float by default).
//                     Logit_value_fma<Tk, V_vec_accum, V_vec_m, INT8_KV_CACHE, FP8_KV_CACHE>(
//                         out, reinterpret_cast<Tk*>(logits_smem + local_time_idx), v_vec, kv_scale_quant_orig_f,
//                         false);
//                 }
//             }
//         }
...
//             {
//                 // Load partial output
//                 int thread_partial_out_offset = tile_idx * params.batch_size * num_heads *
...
//                     = *reinterpret_cast<const V_vec_k*>(&params.partial_out[thread_partial_out_offset + bhi * Dh +
//                     oi]);
