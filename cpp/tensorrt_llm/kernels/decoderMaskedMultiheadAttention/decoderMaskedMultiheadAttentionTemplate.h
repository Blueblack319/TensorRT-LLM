...
inline __device__ float qk_dot_(Q_vec const (&q)[N], K_vec const (&k)[N])
{
#ifdef MMHA_USE_FP32_ACCUM_FOR_FMA
...
template <int THREADS_PER_KEY, typename Q_vec, typename K_vec, int N>
inline __device__ float qk_scale_dot_(Q_vec const (&q)[N], K_vec const (&k)[N], float const k_scale)
{
#ifdef MMHA_USE_FP32_ACCUM_FOR_FMA
    using K_vec_accum = typename K_vec_accum_fp32_<K_vec>::Type;
#else
...
struct Qk_dot
{
    ...
    static inline __device__ float dot(Q_vec const (&q)[N], K_vec const (&k)[N])
    {
        return qk_dot_<THREADS_PER_KEY>(q, k);
    }

    template <typename Q_vec, typename K_vec, int N>
    static inline __device__ float scale_dot(Q_vec const (&q)[N], K_vec const (&k)[N], float const k_scale)
    {
#ifdef MMHA_USE_HMMA
...
inline __device__ float qk_hmma_dot_(K_vec const (&q)[N], K_vec const (&k)[N])
{
