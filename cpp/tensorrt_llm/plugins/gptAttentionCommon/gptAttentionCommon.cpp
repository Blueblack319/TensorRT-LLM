    ...
    nvinfer1::DataType type, int32_t max_num_seq, int32_t max_attention_window, int32_t max_num_tokens) const noexcept
{
    ...
    // [x] Get the size of the intermediate buffers
    size_t const MAX_K = 10;
    size_t const qk_values_size = sizeof(float) * batch_beam * mNumHeads * max_attention_window;
    size_t const qk_max_values_size = sizeof(float) * batch_beam * mNumHeads;
    size_t const topk_qk_indices_size = sizeof(int) * batch_beam * mNumHeads * MAX_K;
    size_t const kv_cache_full_size = sizeof(float) * batch_beam * 2 * 1 * local_hidden_units_kv;

    // [x] Add the space for the intermediate buffers
    int const NUM_BUFFERS = 4 + 4;
    size_t workspaces[NUM_BUFFERS];
    ...
    EnqueueGenerationParams<T, KVCacheBuffer> const& params, cudaStream_t stream)
{
    ...
    // [x] Set the size of intermediate buffers
    int const MAX_K = 10;
    size_t const qk_values_size = sizeof(float) * batch_beam * mNumHeads * params.max_attention_window;
    size_t const qk_max_values_size = sizeof(float) * batch_beam * mNumHeads;
    size_t const topk_qk_indices_size = sizeof(int) * batch_beam * mNumHeads * MAX_K;
    size_t const kv_cache_full_size = sizeof(float) * batch_beam * 2 * 1 * num_kv_heads * head_size;
