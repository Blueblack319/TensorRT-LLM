 ...
 */
#include "gptAttentionCustomPlugin.h"
...
using tensorrt_llm::plugins::GPTAttentionCustomPluginCreator;
using tensorrt_llm::plugins::GPTAttentionCustomPlugin;
...
void GPTAttentionCustomPlugin::initEntryIdx()
{
...
GPTAttentionCustomPlugin::IndexType GPTAttentionCustomPlugin::getIdx(const IdxEntry& entry) const
{
...
GPTAttentionCustomPlugin* GPTAttentionCustomPlugin::clone() const noexcept
{
    return dynamic_cast<GPTAttentionCustomPlugin*>(this->cloneImpl<GPTAttentionCustomPlugin>());
}
...
//     head_size]
nvinfer1::DimsExprs GPTAttentionCustomPlugin::getOutputDimensions(
...
bool GPTAttentionCustomPlugin::supportsFormatCombination(
...
void GPTAttentionCustomPlugin::configurePlugin(const nvinfer1::DynamicPluginTensorDesc* in, int nbInputs,
    const nvinfer1::DynamicPluginTensorDesc* out, int nbOutputs) noexcept
{
    TLLM_CHECK(mHeadSize > 0);
}

// [x] Expand workspace for the qk_values and qk_max_values
// It is done by the function of getWorkspaceSizeForGeneration
size_t GPTAttentionCustomPlugin::getWorkspaceSize(const nvinfer1::PluginTensorDesc* inputs, int nbInputs,
    const nvinfer1::PluginTensorDesc* outputs, int nbOutputs) const noexcept
{
    ...
    // [x] Use the custom getWorkspaceSizeForGeneration create from function overloading
    // Set the value without considering cross_attention
    const int max_attention_window_size = inputs[getIdx(IdxEntry::CACHE_INDIR)].dims.d[2];
    size_t const generation_workspace_size
        = getWorkspaceSizeForGeneration(type, total_num_seq, max_attention_window_size);
...
int GPTAttentionCustomPlugin::enqueueImpl(const nvinfer1::PluginTensorDesc* inputDesc,
...
template <typename T, typename KVCacheBuffer>
int GPTAttentionCustomPlugin::enqueueSome(int32_t seqIdxBeg, int32_t localNbSeq, int32_t tokenIdxBeg,
    int32_t localNbTokens, const nvinfer1::PluginTensorDesc* inputDesc, const nvinfer1::PluginTensorDesc* outputDesc,
...
template <typename T>
int GPTAttentionCustomPlugin::enqueueDispatchKVCacheType(const nvinfer1::PluginTensorDesc* inputDesc,
    const nvinfer1::PluginTensorDesc* outputDesc, const void* const* inputs, void* const* outputs, void* workspace,
    cudaStream_t stream)
{
    if (mPagedKVCache)
...
nvinfer1::DataType GPTAttentionCustomPlugin::getOutputDataType(
...
const char* GPTAttentionCustomPlugin::getPluginType() const noexcept
{
    return GPT_ATTENTION_PLUGIN_NAME;
}

const char* GPTAttentionCustomPlugin::getPluginVersion() const noexcept
{
    return GPT_ATTENTION_PLUGIN_VERSION;
}

int GPTAttentionCustomPlugin::getNbOutputs() const noexcept
{
    return (mPagedKVCache || !useKVCache()) ? 1 : 2;
}

size_t GPTAttentionCustomPlugin::getSerializationSize() const noexcept
{
    return GPTAttentionPluginCommon::getCommonSerializationSize();
}

void GPTAttentionCustomPlugin::serialize(void* buffer) const noexcept
{
...
GPTAttentionCustomPluginCreator::GPTAttentionCustomPluginCreator()
...
const char* GPTAttentionCustomPluginCreator::getPluginName() const noexcept
{
    return GPT_ATTENTION_PLUGIN_NAME;
}

const char* GPTAttentionCustomPluginCreator::getPluginVersion() const noexcept
{
    return GPT_ATTENTION_PLUGIN_VERSION;
}

const PluginFieldCollection* GPTAttentionCustomPluginCreator::getFieldNames() noexcept
{
    return &mFC;
}

IPluginV2* GPTAttentionCustomPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) noexcept
{
    PluginFieldParser p{fc->nbFields, fc->fields};

    try
    {
        auto* obj = new GPTAttentionCustomPlugin(p.getScalar<int32_t>("num_heads").value(),
...
IPluginV2* GPTAttentionCustomPluginCreator::deserializePlugin(
    const char* name, const void* serialData, size_t serialLength) noexcept
{
    // This object will be deleted when the network is destroyed, which will
    // call GPTAttentionCustomPlugin::destroy()
    try
    {
        auto* obj = new GPTAttentionCustomPlugin(serialData, serialLength);
        obj->setPluginNamespace(mNamespace.c_str());
        return obj;
    }
    catch (const std::exception& e)
